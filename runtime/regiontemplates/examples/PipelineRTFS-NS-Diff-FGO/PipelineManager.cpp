...
void add_arguments_to_stages(map<int, PipelineComponentBase*>& merged_stages,
                             ...
                             string name = "tile");
void generate_pre_defined_stages(
    FILE* parameters_values_file, map<int, ArgumentBase*> args,
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1 && string(argv[1]).compare("-h") == 0) {
        cout << "usage: ./PipelineRTFS-NS-Diff-FGO -b <max bucket size> -dkt "
                "<dakota output file> -ma <merging algorithm> [-f] [-ncg] [-nn "
                "<number of nodes>]"
             << endl;
        cout << "   -f    - shuffle" << endl;
        cout << "   -ncg  - don't do coarse grain merging" << endl;
        cout << "   -nn X - notify that X nodes are used to execute the "
                "workflow (only usable for -ma 6)"
             << endl;
        cout << "   -b:" << endl;
        cout << "    + for -ma 0-4 represents the maximum bucket size" << endl;
        cout << "    + for -ma 5   represents the number of buckets generated"
             << endl;
        cout << "    + for -ma 6   represents the maximum number of tasks per "
                "bucket allowed"
             << endl;
        cout << endl;
        ...
        cout << "4 - Double-prunning reuse-tree fine grain merging algorithm"
             << endl;
        cout << "5 - Task-Balanced Reuse-tree fine grain merging algorithm"
             << endl;
        cout << "6 - Task-Balanced Reuse-tree fine grain merging algorithm "
                "with task restriction"
             << endl;
        return 0;
    }
    // if (argc < 7) {
    // 	cout << "usage: ./PipelineRTFS-NS-Diff-FGO -b <max bucket size> -dkt
    // <dakota output file> -ma <merging algorithm> [-f] [-ncg]" << endl;
    // 	return 0;
    // }
    ...
    if (mpi_rank == mpi_size - 1) {
        ...
        bool use_coarse_grain =
            find_arg_pos("-ncg", argc, argv) == -1 ? true : false;
        ...
        // get all workflow inputs without their values, returning also the
        // parameters values (i.e list<ArgumentBase> values) on another map
        ...
        // get all stages, also setting the uid from this context to Task (i.e
        // Task::setId()) also returns the list of arguments used the stages
        // dependencies are also set here (i.e Task::addDependency())
        map<int, PipelineComponentBase*> base_stages;
        map<int, ArgumentBase*> interstage_arguments;
        get_stages_from_file(workflow_descriptor, base_stages,
                             interstage_arguments);
        ...
        connect_stages_from_file(workflow_descriptor, base_stages,
                                 interstage_arguments, workflow_inputs, deps,
                                 workflow_outputs);
        ...
        generate_pre_defined_stages(parameters_values_file, args, base_stages,
                                    workflow_outputs, expanded_args,
                                    expanded_stages, use_coarse_grain);
...
//                           (start.tv_sec * 1000000 + start.tv_usec));
        ...
        for (pair<int, PipelineComponentBase*> p : merged_stages) {
            ...
            for (int i = 0; i < p.second->getNumberDependencies(); i++) {
                // if the i-th dependency of p still exists and is reused, add
                // the dependency of the
                //   reused stage to p
                if (merged_stages.find(p.second->getDependency(i)) !=
                        merged_stages.end() &&
                    merged_stages[p.second->getDependency(i)]->reused != NULL) {
                    deps_tmp.emplace_back(
                        merged_stages[p.second->getDependency(i)]
                            ->reused->getId());
                }
            }
            for (int d : deps_tmp) p.second->addDependency(d);

            // connect correct output arguments
            bool updated = true;
            // cout << "Updating arguments of " << p.second->getId() << ":" <<
            // p.second->getName() << endl;
            while (updated) {
                updated = false;
                for (ArgumentBase* a : p.second->getArguments()) {
                    if (a->getParent() != 0 &&
                        merged_stages[a->getParent()]->reused != NULL) {
                        // cout << "Replacing argument " << endl;
                        updated = true;
                        p.second->replaceArgument(
                            a->getId(),
                            merged_stages[a->getParent()]
                                ->reused->getArgumentByName(a->getName()));
                        break;
                    }
                }
            }
        }
        // DEBUG_PCBLIST_TO_DOT("test12", merged_stages);
        ...
        for (pair<int, PipelineComponentBase*> s : merged_stages) {
            if (s.second->reused == NULL) {
                ...
                generate_drs(((RTPipelineComponentBase*)s.second)
                                 ->getRegionTemplateInstance("tile"),
                             s.second, expanded_args);

                cout << "sent component " << s.second->getId() << ":"
                     << s.second->getName() << " sized " << s.second->size()
                     << " to execute with args:" << endl;
                // cout << "\tall args: " << endl;
                // for (ArgumentBase* a : s.second->getArguments())
                // 	cout << "\t\t" << a->getId() << ":" << a->getName() << " = "
                // 		<< a->toString() << " parent " << a->getParent() <<
                // endl; cout << "\tinputs: " << endl; for (int i :
                // s.second->getInputs()) 	cout << "\t\t" << i << ":" <<
                // expanded_args[i]->getName() << " = "
                // 		<< expanded_args[i]->toString() << " parent " <<
                // expanded_args[i]->getParent() << endl; cout << "\toutputs: "
                // << endl; for (int i : s.second->getOutputs()) 	cout <<
                // "\t\t" << i << ":" << expanded_args[i]->getName() << " = "
                // 		<< expanded_args[i]->toString() << endl;
                cout << "\tdependencies:" << endl;
                for (int i = 0; i < s.second->getNumberDependencies(); i++)
                    cout << "\t\t"
                         << merged_stages[s.second->getDependency(i)]->getId()
                         << ":"
                         << merged_stages[s.second->getDependency(i)]->getName()
                         << endl;
                cout << "\ttasks:" << endl;
                ...
                // workaround to make sure that the RTs, if any, won't leak on
        ...
        long run_time = ((end.tv_sec * 1000000 + end.tv_usec) -
                         (start.tv_sec * 1000000 + start.tv_usec));
        ofstream run_time_f(
            dakota_file + "-b" + to_string(max_bucket_size) + "run_time.log",
            ios::app);
        run_time_f << run_time << "\t";
        run_time_f.close();
        ...
        for (pair<int, ArgumentBase*> output : workflow_outputs) {
            // cout << "\t" << output.second->getName() << ":" <<
            // output.second->getId() << " = " <<
            // 	sysEnv.getComponentResultData(output.second->getId()) << endl;
            char* resultData =
                sysEnv.getComponentResultData(output.second->getId());
            std::cout << "Diff Id: " << output.second->getId()
                      << " resultData -  ";
            if (resultData != NULL) {
        ...
        do {
            MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &flag,
                       MPI_STATUS_IGNORE);
            if (flag) usleep(200000);
        } while (!flag);
        MPI_Recv(&mpi_val, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG,
    ...
    for (pair<int, ArgumentBase*> p : expanded_args) {
        ...
        if (p.second->getType() == ArgumentBase::RT) {
            if (((ArgumentRT*)p.second)->isFileInput) {
                // create the data region and add it to the input region
                // template
...
void add_arguments_to_stages(
    std::map<int, PipelineComponentBase*>& merged_stages,
    ...
    for (pair<int, PipelineComponentBase*>&& stage : merged_stages) {
        ...
        for (int arg_id : stage.second->getInputs()) {
            ...
            if (new_arg->getType() == ArgumentBase::RT) {
                // std::cout << "input RT : " <<
                // merged_arguments[arg_id]->getName() << std::endl; insert the
                // region template on the parent stage if the argument is a DR
                // and if the RT wasn't already added
                if (((RTPipelineComponentBase*)stage.second)
                        ->getRegionTemplateInstance(rt->getName()) == NULL) {
                    ((RTPipelineComponentBase*)stage.second)
                        ->addRegionTemplateInstance(rt, rt->getName());
                }
                ((RTPipelineComponentBase*)stage.second)
                    ->addInputOutputDataRegion(rt->getName(),
                                               new_arg->getName(),
            ...
            if (merged_arguments[arg_id]->getParent() != 0) {
                ...
                // std::cout << "[before]Dependency: " << stage.second->getId()
                // << ":" << stage.second->getName()
                // 	<< " ->addDependency( " << parent << " )" << std::endl;
                if (merged_stages[merged_arguments[arg_id]->getParent()]
                        ->reused != NULL)
                    parent =
                        merged_stages[merged_arguments[arg_id]->getParent()]
                            ->reused->getId();
                // std::cout << "Dependency: " << stage.second->getId() << ":"
                // << stage.second->getName()
                // 	<< " ->addDependency( " << parent << " )" << std::endl;
            ...
            if (merged_arguments[arg_id]->getType() == ArgumentBase::RT) {
                ...
                if (((RTPipelineComponentBase*)stage.second)
                    ...
                    ((RTPipelineComponentBase*)stage.second)
                        ->addRegionTemplateInstance(rt, rt->getName());
                ((RTPipelineComponentBase*)stage.second)
                    ->addInputOutputDataRegion(
                        rt->getName(), merged_arguments[arg_id]->getName(),
...
bool all_inps_in(const list<int>& inps, const map<int, ArgumentBase*>& args,
    ...
    for (int i : inps) {
        // cout << "checking arg " << args.at(i)->getId() << ":" <<
        // args.at(i)->getName() << endl;
        if (input_arguments.find(args.at(i)->getName()) ==
                input_arguments.end() &&
...
bool all_inps_in(list<int> inps, map<int, list<ArgumentBase*>> ref) {
    for (int i : inps) {
        if (ref.find(i) == ref.end()) return false;
    }
    return true;
}
...
void generate_pre_defined_stages(
    ...
    map<int, ArgumentBase*>& workflow_outputs,
    map<int, ArgumentBase*>& expanded_args,
    map<int, PipelineComponentBase*>& expanded_stages, bool use_coarse_grain,
    bool clustered_generation) {
    ...
    while (getline(&line, &length, parameters_values_file) != -1) {
        ...
        while (true) {
            ...
            // verify if the argument is on the map and if it contains the
            // current value
            bool found = false;
            for (ArgumentBase* a : input_arguments[name]) {
                ArgumentBase* aa = gen_arg(value, type);

                // cout << "[---] comparing " << a->getName() << ":" <<
                // a->toString() << " with " << name << ":" << aa->toString() <<
                // endl;
                if (a->getName().compare(name) == 0 &&
                    a->toString().compare(aa->toString()) == 0) {
                    found = true;
                    arg = a;
                    delete aa;
                    break;
    ...
    while (base_stages.size() != 0) {
        ...
        for (pair<int, PipelineComponentBase*> p : base_stages) {
            // attempt to find a stage witch has all inputs expanded either on
            // the workflow inputs or interstage ones cout << "checking stage "
            // << p.second->getName() << endl;
            if (all_inps_in(p.second->getInputs(), args, input_arguments,
                            args_values)) {
                // cout << "stage " << p.second->getName() << " has all inputs"
                // << endl;

                // A list of concatenated arg values, used as a quick way to
                // verify if a stage with the same args was already created.
                ...
                for (pair<int, list<ArgumentBase*>> as : stages_arguments) {
                    ...
                    string arg_values = "";
                    // add all arguments from stages_arguments that belong to
                    // stage p.second
                    for (int inp_id : p.second->getInputs()) {
                        // cout << "checking input " << args[inp_id]->getName()
                        // << " of " << p.second->getName() << " with " <<
                        // as.second.size() << " parameters" << endl;
                        for (ArgumentBase* a : as.second) {
                            // cout << "checking arg " << a->getName() << ":" <<
                            // a->getId() << endl;
                            if (args.at(inp_id)->getName().compare(
                                    a->getName()) == 0) {
                                arg_values += to_string(a->getId());
                                tmp->addInput(a->getId());
                                // tmp->addArgument(a->clone());
                                // cout << "added arg " << a->getName() << ":"
                                // << a->getId() << " = " << a->toString() <<
                                // endl;
                                break;
                            }
                    ...
                    map<string, PipelineComponentBase*>::iterator it =
                        arg_values_list.find(arg_values);
                    // verify if there is no other stage with the same values
                    if (it == arg_values_list.end() || !use_coarse_grain) {
                        // add current stage and args values to be compared
                        // later
                        ...
                        for (int out_id : p.second->getOutputs()) {
                            ...
                            // add stage's output arguments to current
                            // workflow's argument list
                        ...
                        for (int out_id : it->second->getOutputs()) {
                            // add reused stage's output arguments to current
                            // workflow's argument list cout << "reusing stage "
                            // << out_id << " from workflow "
                            // 	<< it->second->getId() << endl;
                            stages_arguments[as.first].emplace_back(
                                find_argument(args_values, out_id));
                        }
                ...
                // remove stage descriptor since it was already solved and break
                // the loop
    ...
    while (workflow_outputs_cpy.size() != 0) {
        ...
        // add a copy of the old arg with the correct id to the final map for
        // each repeated output cout << "checking output " << old_arg->getId()
        // << ":" << old_arg->getName() << endl;
        for (ArgumentBase* a : args_values) {
            // cout << "comparing with output " << a->getId() << ":" <<
            // a->getName() << endl;
...
// map<int, list<ArgumentBase*>> args_values: the list of values for each
// argument. 		this map will be changed inside and must start with all
// inputs map<int, ArgumentBase*> expanded_args: output of function. map with
// all args to be used on execution. map<int, PipelineComponentBase*> stages:
// map of all stages with arguments mapped to args map<int,
// PipelineComponentBase*> expanded_stages: output of function. Returns all
// stages 		ready for execution.
void expand_stages(const map<int, ArgumentBase*>& args,
    ...
    // 	// cout << "base argument " << p.first << ":" <<
    // args.at(p.first)->getName() << endl; 	for (ArgumentBase* a : p.second)
    // { 		a->setId(new_uid());
    ...
    while (stages.size() != 0) {
        ...
        for (pair<int, PipelineComponentBase*> p : stages) {
            ...
            if (all_inps_in(p.second->getInputs(), args_values)) {
                // cout << "stage " << p.second->getName() << " has all inputs"
                // << endl; create temporary list of stages to be expanded
                list<PipelineComponentBase*> stages_iterative;
                // starts the stages temp list with the current stage without
                // the valued arguments
                stages_iterative.emplace_back(p.second);

                // cout << "expanding stage " << p.second->getName() << " with
                // inputs:" << endl; for (int inp_id : p.second->getInputs())
                // 	cout << "\t" << args.at(inp_id)->getName() << endl;

                // expands all inputs from stage p
                for (int inp_id : p.second->getInputs()) {
                    // cout << "expanding input " << args.at(inp_id)->getName()
                    // << " with values:" << endl; for (ArgumentBase* a :
                    // args_values[inp_id]) 	cout << "\t" << a->toString() <<
                    // endl; expands the values of each inptut in p->getInputs()
                    ...
                    for (ArgumentBase* a : args_values[inp_id]) {
                        ...
                        // 	cout << "\t" << pp->getId() << ":" << pp->getName()
                        // << endl; generate a copy of the current stage for
                        // each input-value pair
                        for (PipelineComponentBase* pt : stages_iterative) {
                            // cout << "updating stage " << pt->getId() << ":"
                            // << pt->getName() << endl; clone pt basic
                            // infoinfoinfo
                            ...
                            // cout << "all outputs from stage " <<
                            // pt->getName() << endl; for (int out :
                            // pt->getOutputs()) 	cout << "\t" << out << endl;
                ...
                // for (pair<int, list<ArgumentBase*>> p : args_values) {
                // 	cout << "base argument " << p.first << ":" <<
                // args.at(p.first)->getName() << endl; 	for (ArgumentBase* a
                // : p.second) { 		cout << "\t" << a->getId() << ":" <<
                // a->getName() << " = " << a->toString() << endl;
                // 	}
                // }
            ...
            // else
            // cout << "stage " << p.second->getName() << " have unmet
            // dependencies " << endl;
        }
    }
    ...
    while (workflow_outputs_cpy.size() != 0) {
        ...
        // get the list of parameters (i.e the number of copies and the final
        // ids) of the outputs
